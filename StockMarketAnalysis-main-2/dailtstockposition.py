...
#def Connection(tickerlist,strategies);
#    try:
#        cnxion = mysql.connector.connect(user='root', password = 'Godly@008', host'127.0.0.1', database='Ticker')
#        print("Connection SUCCESS WAWAWEWA ")
#    except mysql.connector.Error as error:
#        print("Failed to CONNECT GYPSIES: {}".format(error))
#    finally:
#        if (connection.is_connected()):
##            n = len(tickerlist)
##            m = len(strat)
#            cursor = cnxion.cursor()
#            result = cursor.execute(User)
#            inUser = ("INSERT INTO User" "(TickerName, Startegies)"
#                        "VALUES (%s,%s)")
#
#            for i in ticketlist:
#                for k in strategies:
#                    dataUser = (tickerlist[i],startegies[k])
#                    cursor.execute(inUser, dataUser)
#                    Id = cursor.lastrowid
#
#                    break;
#
...
def topTicks():
   ...
   # file = open("tick.txt","r")
   # list = file.read()
#    data = list.splitlines()
#    file.close()
#    nList = [tuple(map(int, sub.split('\n'))) for sub in data]
#    print(nList)
